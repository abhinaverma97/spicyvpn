...
# SpicyVPN Low-Latency Optimized Test Node Installer
# Fully isolated - no master, no fleet, no agent.
# 1:1 identical Xray config to production node, with gaming low-latency kernel tuning.

echo "=========================================="
echo "  SpicyVPN Gaming-Optimized Test Node"
echo "  (Low-Latency & Anti-Bufferbloat TCP)"
echo "=========================================="
echo ""
...
HOST="${DOMAIN:-$IP}"
SERVICE_NAME="spicyvpn-opt-grpc"
...
CONFEOF
echo "✅ Xray low-latency config written"
...
# --- systemd service ---
cat > /etc/systemd/system/xray-opt-test.service <<SERVICEOF
[Unit]
Description=SpicyVPN Gaming-Optimized Test Node (Xray)
...
systemctl enable xray-opt-test.service 2>/dev/null
systemctl restart xray-opt-test.service
echo "✅ Service xray-opt-test started"

# --- Verify ---
sleep 2
if systemctl is-active --quiet xray-opt-test; then
  echo "✅ Xray is RUNNING"
else
  echo "❌ Xray failed to start - check: journalctl -u xray-opt-test -n 30"
fi
...
# --- VLESS link ---
VLESS_LINK="vless://${UUID}@${HOST}:8444?security=tls&sni=google.com&alpn=h2,http/1.1&fp=chrome&type=grpc&serviceName=${SERVICE_NAME}&allowInsecure=1#SpicyVPN-GamingOpt-${HOST}"

echo ""
echo "=========================================="
echo "  GAMING-OPTIMIZED TEST NODE READY"
echo "=========================================="
echo ""
echo "  Host:       ${HOST}"
echo "  Port:       8444"
...
echo "    systemctl stop xray-opt-test"
echo "    systemctl disable xray-opt-test"
echo "    rm -rf /usr/local/etc/xray /usr/local/bin/xray"
echo "    rm /etc/systemd/system/xray-opt-test.service"
echo "    rm /etc/sysctl.d/99-spicyvpn-opt.conf"
echo "=========================================="
