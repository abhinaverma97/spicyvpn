...
export function generateHysteriaLink(uuid: string, serverIp?: string, nodeName?: string): string {
  const ipToUse = serverIp || FALLBACK_SERVER_IP;
  const nameToUse = nodeName ? `SpicyVPN - ${nodeName}` : "SpicyVPN";
  ...
  return `hysteria2://${uuid}@${ipToUse}:${PORT}?insecure=1&mport=20000-50000&upmbps=8&downmbps=5&sni=${SNI}#${encodeURIComponent(nameToUse)}`;
}
