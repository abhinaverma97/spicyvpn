...
A standalone installer that sets up an **isolated Xray node** on any VPS without connecting to the SpicyVPN fleet. Useful for testing config changes, evaluating VPS providers, or benchmarking.
...
### Complete cleanup
```bash
sudo systemctl stop xray-test 2>/dev/null; sudo systemctl disable xray-test 2>/dev/null; sudo rm -rf /usr/local/etc/xray /usr/local/bin/xray /etc/systemd/system/xray-test.service /etc/sysctl.d/99-spicyvpn-test.conf && sudo systemctl daemon-reload
```
